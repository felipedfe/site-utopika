...
import React, { useEffect, useState, useContext } from 'react';
import { db, doc, getDoc } from '../../firebase'; // Importando Firestore
import myContext from '../../context/myContext';
import { textLanguages as textOptions } from '../../data/languages';
import { AiOutlineInstagram as InstagramIcon } from 'react-icons/ai';
import './insta-feed.css';
...
function InstaFeed() {
  ...
  const getInstaFeed = async () => {
    try {
      ...
      const tokenDoc = await getDoc(doc(db, "tokens", "instagram"));
      if (!tokenDoc.exists()) throw new Error("Token não encontrado no Firestore.");
